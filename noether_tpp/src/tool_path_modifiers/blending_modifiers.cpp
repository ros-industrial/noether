...
ToolPaths AngledOrientationModifier::modify(ToolPaths tool_paths) const
{
  for (ToolPath& tool_path : tool_paths)
  {
    ...
    // Sign is determined by whether the first point's y-axis is aligned with the nominal path y-axis (i.e., the cross of the waypoint's z-axis with the nominal path direction)
...
ToolPaths CircularLeadInModifier::modify(ToolPaths tool_paths) const
{
  ...
  for (ToolPath& tool_path : tool_paths)
  {
    ...
    for (ToolPathSegment& segment : tool_path)
    {
      ...
      {        Eigen::Isometry3d pt = radius_center * Eigen::AngleAxisd(sign * delta_theta * (i + 1), Eigen::Vector3d::UnitY()) * Eigen::Translation3d(0, 0, -arc_radius_);
