...
PluginLoaderWidget<PluginT>::PluginLoaderWidget(std::shared_ptr<const WidgetFactory> factory,
                                                const QString& title,
                                                QWidget* parent)
  : BaseWidget(parent), ui_(new Ui::PluginLoader()), factory_(factory)
{
  ui_->setupUi(this);

  QStringList plugins = toQStringList(factory_->getAvailablePlugins<PluginT>());
  plugins.sort();
  ...
  // Remove all widgets
  connect(ui_->tool_button_remove_all, &QAbstractButton::clicked, this, &PluginLoaderWidget::removeWidgets);
...
void PluginLoaderWidget<PluginT>::save(YAML::Node& config) const
{
  for (int i = 0; i < ui_->stacked_widget->count(); ++i)
  {
    auto widget = dynamic_cast<const BaseWidget*>(ui_->stacked_widget->widget(i));
    if (widget)
    {
