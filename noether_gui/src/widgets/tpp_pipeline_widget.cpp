...
namespace noether
{
TPPPipelineWidget::TPPPipelineWidget(std::shared_ptr<const boost_plugin_loader::PluginLoader> loader, QWidget* parent)
  : QWidget(parent)
  , loader_(loader)
  , ui_(new Ui::TPPPipeline())
  , mesh_modifier_loader_widget_(new PluginLoaderWidget<MeshModifierWidgetPlugin>(loader, "Mesh Modifier", this))
  , tool_path_modifier_loader_widget_(
        new PluginLoaderWidget<ToolPathModifierWidgetPlugin>(loader, "Tool Path Modifier", this))
{
  ui_->setupUi(this);
  ...
  for (const QString& plugin_name : plugin_names)
  {
    ...
    else
    {
      ...
      ui_->stacked_widget->addWidget(plugin->create({}, loader_, this));
    }
  }
...
ToolPathPlannerPipeline TPPPipelineWidget::createPipeline() const
{
  YAML::Node config;
  save(config);
  return ToolPathPlannerPipeline(loader_, config);
}
